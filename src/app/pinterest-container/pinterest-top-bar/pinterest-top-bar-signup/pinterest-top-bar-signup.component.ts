...
export class PinterestTopBarSignupComponent implements OnInit {
  ...
  public ngOnInit(): void {
    this.signUpForm = this.fb.group({
      ...
      confirmPassword: ['', Validators.compose([Validators.required, this.isPasswordSame()])],
      termsAgreement: ['', Validators.compose([Validators.required])]
    })
  ...
  private isPasswordSame(): ValidatorFn | null {
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      if (!control.value) {
        return null;
      }
      return this.signUpForm.controls['password'].value === control.value ? null : { notSamePassword: true };
    };
  }

  public isTermsAgreed(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: boolean } | null => {
      if (!control.value) {
        return null;
      }
      return this.signUpForm.get('termsAgreement')?.value === true ? null : { notValidated : true };
    };
